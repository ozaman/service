...
if($order->type == 'transfer'){  	
	if($product->cartype == '2'){  
		...
		if($add_m < 60){ $time_of_join =  " ~ ".trim($order->airout_h).".".$add_m; }
	}	  	
}
//////////////////////// Tour
if($order->type <> 'transfer'){ 
  ...
  public function book_create_data($data) {
    ...
    if($data['product_type'] == 'transfer') {
      $admin = $this->db->select('admin_company,cartype,stay,area')->from('web_transferproduct')->where('id',$data['product_id'])->get()->row();
      $cartype_transfer = $admin->cartype;
      $province_id = $admin->stay;
      /////////// 1. Start Check Rental  update 2017-01-11 16:08
      if($data['agent'] == '13' || $data['agent'] == 1563) {
	      if($admin->area == 'Rental'){
		  	$data['number_car'] = $data['pax'];
		  }
	  }
	  /////////// 1. End Check Rental  update 2017-01-11 16:08
    }
    else {
      /////////////  Tour product Zone
...
$date_p_3 = date('Y-m-d', strtotime($date .' +3 day'));
    ...
    if($q-> sub_reject  == 1){
      ...
      $this->db->where('ref',$id)->update('web_order',$update);
      $DB2 = $this->load->database('admin_acc', TRUE);
...
*/      
    ...
    if($data['agent'] == '13'){
        ...
        /////////// 2. Start Check Rental  update 2017-01-11 16:08
        if($q->cartype == 2) {
          if($data['agent'] == '13' || $data['agent'] == 1563) {
	          if($q->area == 'Rental') {
	          	$number_car = $data['pax'];
        ...
        /////////// 2. End Check Rental  update 2017-01-11 16:08
