...
if(($params['place_default']==!"" or $params['place_default']==!NULL) and ($params['place_default_to']==!"" or $params['place_default_to']==!NULL)){
	...
	/*aum_from --> aum_to*/
			...
			$this->db->where('a.aum_to',''.$data_aum_to.'');
			$this->db->where('a.aum_from',''.$data_aum_from.'');
			$this->db->group_by('a.id'); 
			$query = $this->db->get(); 
			if($query->num_rows() > 0) {
			      foreach($query->result() as $row) {
	...
	foreach ($select as $key => $value){
			...
			$this->db->join('web_gallerycar e', 'e.category=b.car_model', 'left');
			$this->db->join('web_transferproduct_utf f', 'f.id=a.id', 'left');
			$this->db->where('aum_from',''.$data_aum_from.'');
			$this->db->where('place_default_to',''.$place_default_to.'');
			$this->db->group_by('a.id'); 
			$query = $this->db->get(); 
			if($query->num_rows() > 0) {
	...
	foreach ($select as $key => $value){
			...
			$this->db->join('web_transferproduct_utf f', 'f.id=a.id', 'left');
			$this->db->where('place_default',''.$place_default.'');
			$this->db->where('aum_to',''.$data_aum_to.'');
			$this->db->group_by('a.id'); 
			$query = $this->db->get(); 
			if($query->num_rows() > 0) {
			      foreach($query->result() as $row) {
...
$arrayname['status'] =  "202";
		...
		foreach ($select as $key => $value){
			...
			$this->db->join('web_gallerycar e', 'e.category=b.car_model', 'left');
			$this->db->join('web_transferproduct_utf f', 'f.id=a.id', 'left');
			$this->db->where('stay',''.$data_pro_from.'');
			$this->db->where('stay_to',''.$data_pro_to.'');
			$this->db->group_by('a.id'); 
			$this->db->limit(1); 
			$query = $this->db->get(); 
			if($query->num_rows() > 0) {
