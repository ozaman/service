...
class Sendback extends CI_Controller {
  ...
  public function index($token) {
    ...
    if($method != 'POST' ) {
      ...
      $response = $this->Model_sendback->check_agent($token);
      if($response['status'] == 200) {
        $post = json_decode(file_get_contents('php://input'), TRUE);
        //$post = $_GET['agent_ref'];
        $id = $this->input->get_request_header('agent_ref', TRUE);
        $params['agent'] = $response['agent'];
        $resp = $this->Model_sendback->book_detail_data($id,$params);
